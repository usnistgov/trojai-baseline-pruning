...
##################################################################
def remove_prune_model(model, model_name, output_transform, sample_shift, sampling_method, ranking_method, probability, num_shifts):
    model.cpu()  # -- PB
    print('INFO: model_name ', model_name)
    print('model.modules():', model.modules())
    print('model:', model)
    if 'GRU' in model_name:
        #tens = torch.randn(1, 768, 256)
        tens = torch.randn(1, 768, 768)
        # GruLinearModel(
    ...
    def remove_prune_random(conv, pruned_prob):
    ...
    def remove_prune_targeted(sample_shift, conv, pruned_prob, ranking_method,num_shifts) :
        weight = conv.weight.detach().cpu().numpy()
        out_channels = weight.shape[0]
    ...
    def remove_prune_uniform(sample_shift, conv, pruned_prob, ranking_method, num_shifts):
        weight = conv.weight.detach().cpu().numpy()
        out_channels = weight.shape[0]
        ...
        for layer_idx in range (0,num_layers):
            ...
            for gate_idx in range(0, num_gatesPerLayer):
                ...
                if 'random' in sampling_method:
                    num = remove_prune_random(layer_to_prune.all_weights[layer_idx][gate_idx], multiplier)
                elif 'targeted' in sampling_method:
                    num = remove_prune_targeted(sample_shift, layer_to_prune.all_weights[layer_idx][gate_idx],
                                               multiplier, ranking_method, num_shifts)
                elif 'uniform' in sampling_method:
                    num = remove_prune_uniform(sample_shift, layer_to_prune.all_weights[layer_idx][gate_idx], multiplier, ranking_method, num_shifts)
                else:
