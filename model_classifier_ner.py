...
""" 
This class returns the AI architecture for NER problems in TrojAI Round 7

From: https://pages.nist.gov/trojai/docs/data.html#round-7
The embeddings used are drawn from HuggingFace.
EMBEDDING_LEVELS = ['MobileBERTLinear', 'DistilBERTLinear','BERTLinear', 'RoBERTaLinear']

"""
# round 7 trojAI challenge

class model_classifier:
    # round 7
    # centroid file size values per architecture model
    mobileBERT_linear_model_size = float(98.86031025)
    distilBERT_linear_model_size = float(260.844357)
    BERT_linear_size = float(435.7018681)
    RoBERTa_model_size = float(496.3553596)

    NerLinear_model_size = float(322.9404737) # average of 192 training models

    MODEL_ARCHITECTURES = ['NerLinearModel']
    NUMBER_OF_TYPES_PER_ARCH = [4]
    MODEL_NAMES = ['MobileBERTLinear', 'DistilBERTLinear','BERTLinear', 'RoBERTaLinear']
    MODEL_REF_SIZES = [mobileBERT_linear_model_size, distilBERT_linear_model_size, BERT_linear_size, RoBERTa_model_size]
    MODEL_SIZE_STDEV = [0.005930291, 0.00592737, 0.005916521, 0.005979864]
    ...
    def classify_architecture(self, mydevice):
        ...
        #model_orig = torch.load(self.model_filepath, map_location=mydevice)
        # get the model class name <class 'transformers.models.mobilebert.tokenization_mobilebert_fast.MobileBertTokenizerFast'>
        ...
        split_string = model_name_str.split("model_factories.") # transformers.models.
        # print(split_string)
        # second split
    ...
    def get_filesize(self, model_architecture):
        ...
        found_match = False
        min_delta = float('inf')
        for i in range(len(a.MODEL_REF_SIZES)):
