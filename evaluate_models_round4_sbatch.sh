...
RESULT_DIR=/wrk/pnb/trojai_data/round4/scratch_r4-nS$NUM_SAMPLES-nD$NUM_IMAGES-$PRUNING_METHOD
SCRATCH_DIR=/wrk/pnb/trojai_data/round4/scratch_r4-nS$NUM_SAMPLES-nD$NUM_IMAGES-$PRUNING_METHOD
...
do
	...
	if [ "$dir" != "$MODEL_DIR" ]; then
		...
		if [[ $MODEL == id* ]] ; then
			...
			python ./trojan_detector_round4_new.py --model_filepath $dir/model.pt  --result_filepath $RESULT_DIR/test_python_output.txt --scratch_dirpath $SCRATCH_DIR --examples_dirpath $dir/clean_example_data  --num_samples $NUM_SAMPLES --num_images_used $NUM_IMAGES --pruning_method $PRUNING_METHOD
