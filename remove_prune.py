...
"""
This class is designed to remove conv2d and batchnorm filters
"""
...
def prune_model(model, model_name, output_transform, sample_shift, sampling_method, ranking_method, probability, num_shifts):
    model.cpu()  # -- PB
    ...
    DG = tp.DependencyGraph().build_dependency(model, example_inputs=tens,
                                               output_transform=output_transform)

    ############################################
    # methods for random sampling
    def prune_conv_random(conv, pruned_prob):
    ...
    def prune_batchnorm_random(batchnorm, pruned_prob):
        ...
        prune_index = random.sample(list(range(out_channels)), num_pruned)
        plan = DG.get_pruning_plan(batchnorm, tp.prune_batchnorm, prune_index)
        plan.exec()
        return num_pruned

    ############################################
    # methods for targeted sampling with shift
    def prune_conv_targeted(sample_shift, conv, pruned_prob, ranking_method,num_shifts) :
        weight = conv.weight.detach().cpu().numpy()
        out_channels = weight.shape[0]
        ...
        # if sample_shift == 0:
        #     filter_hist_filepath = 'C:/PeterB/Projects/TrojAI/python/trojai-pruning/scratch_r2/number_filters.csv'
    ...
    def prune_batchnorm_targeted(sample_shift,batchnorm, pruned_prob, ranking_method, num_shifts):
        weight = batchnorm.weight.detach().cpu().numpy()
    ...
    def prune_conv_uniform(sample_shift, conv, pruned_prob, ranking_method, num_shifts):
        weight = conv.weight.detach().cpu().numpy()
        out_channels = weight.shape[0]
        ...
        if 'L1' in ranking_method:
            ...
            # mean_weight = np.sum(weight, axis=(1, 2, 3))
            # mean_weight = mean_weight/out_channels
            #
            # stdev_weight = np.sum( (weight - mean_weight)*(weight - mean_weight), axis=(1, 2, 3))
            # stdev_weight = np.swrt(stdev_weight/out_channels)
            # stdev_weight = np.ndarray.flatten(weight) #torch.flatten(weight, start_dim=1)
            # stdev_weight = statistics.stdev(stdev_weight)  # for test print
            # rank_norm = np.sum(stdev_weight, axis=(1, 2, 3))
            rank_norm = np.std(weight, axis=(1, 2, 3))
        else:
            print('ERROR: unrecognized ranking method:', ranking_method)
            return 0
        ...
        if num_pruned > 0:
            ...
            if sample_shift >= num_shifts:
                print('ERROR: index of a sample (sample_shift):',sample_shift, ' is larger or equal to num_samples (num_shifts):',num_shifts)
                return 0
    ...
    def prune_batchnorm_uniform(sample_shift, batchnorm, pruned_prob, ranking_method, num_shifts):
        weight = batchnorm.weight.detach().cpu().numpy()
        # TODO explore computing L1 norm for abs(weight minus 1.0)
        out_channels = weight.shape[0]
        L1_norm = weight  # np.sum(np.abs(weight), axis=(1, 2, 3))
        ...
        if num_pruned > 0:
            ...
            temp_idx = []
            #num_shifts = 5  # TODO this is the number of samples that should be passed to this method
            if sample_shift >= num_shifts:
    ...
    for layer_to_prune in prunable_modules:
        ...
        if isinstance(layer_to_prune, nn.Conv2d):
            if 'random' in sampling_method:
                num = prune_conv_random(layer_to_prune, multiplier)
            elif 'targeted' in sampling_method:
                num = prune_conv_targeted(sample_shift, layer_to_prune, multiplier, ranking_method, num_shifts)
            elif 'uniform' in sampling_method:
                num = prune_conv_uniform(sample_shift, layer_to_prune, multiplier, ranking_method, num_shifts)
            else:
        ...
        if isinstance(layer_to_prune, nn.BatchNorm2d):
            if 'random' in sampling_method:
                num = prune_batchnorm_random(layer_to_prune, multiplier)
            elif 'targeted' in sampling_method:
                num = prune_batchnorm_targeted(sample_shift, layer_to_prune, multiplier, ranking_method, num_shifts)
            elif 'uniform' in sampling_method:
                num = prune_batchnorm_uniform(sample_shift, layer_to_prune, multiplier, ranking_method, num_shifts)
            else:
                print('ERROR: unrecognized sampling method:', sampling_method)
                num = 0
